...
export const DocumentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getDocumentById = (id: string) => {
    ...
    console.log("Finding document with qrId:", qrId);
    
    if (!documents || documents.length === 0) {
      console.log("No documents available in context");
      return undefined;
    }
    
    if (!qrId) {
      console.log("Invalid qrId provided:", qrId);
      return undefined;
    }
    
    // Log all available QR IDs for debugging
    const availableQrIds = documents
      .filter(doc => doc.qrId)
      .map(doc => ({ id: doc.id, qrId: doc.qrId }));
    console.log("Available QR IDs:", availableQrIds);
    
    const doc = documents.find(doc => doc.qrId === qrId);
    console.log("Found document:", doc ? { id: doc.id, title: doc.title, qrId: doc.qrId } : null);
    return doc;
  };
